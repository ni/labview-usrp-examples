...
- LabVIEW Python Node user manual:
...
BlockInfo = collections.namedtuple("BlockInfo", ["block", "port"])

# Named tuple to hold required RFNoC graph settings.
GraphSettings = collections.namedtuple(
    "GraphSettings", ["rx_radio", "rx_chan", "tx_radio", "tx_chan"]
)

# Named tuple to hold required RF settings.
RfSettings = collections.namedtuple("RfSettings", ["frequency", "gain", "antenna", "rate"])
...
class ExampleSession:
    ...
    def setup_graph(self, graph_settings):
        ...
        radio channels as specified in the `graph_settings` parameter. It performs the
        following steps:
        ...
        - Connects the streamers through the discovered block chains,
          handling DDC (for Rx) and DUC (for Tx) blocks if present.
        ...
        edges = uhd.rfnoc.get_block_chain(
            self.graph, self.rx_radio.get_unique_id(), self.rx_chan, True
        )
        if not edges:
            raise RuntimeError(
                f"No chain found for {self.rx_radio.get_unique_id()} ({self.rx_chan})"
            )
        ...
        if "DDC" in last_block_in_chain:
            ...
            self.ddc = BlockInfo(
                uhd.rfnoc.DdcBlockControl(self.graph.get_block(last_block_in_chain)),
                last_port_in_chain,
            )
        else:
            self.ddc = None
        ...
        if self.ddc:
            uhd.rfnoc.connect_through_blocks(
                self.graph,
                self.rx_radio.get_unique_id(),
                self.rx_chan,
                last_block_in_chain,
                last_port_in_chain,
            )

        self.graph.connect(last_block_in_chain, last_port_in_chain, self.rx_streamer, 0)
        ...
        edges = uhd.rfnoc.get_block_chain(
            self.graph, self.tx_radio.get_unique_id(), self.tx_chan, False
        )
        if not edges:
            raise RuntimeError(
                f"No chain found for {self.tx_radio.get_unique_id()} ({self.tx_chan})"
            )
        ...
        if "DUC" in last_block_out_chain:
            ...
            self.duc = BlockInfo(
                uhd.rfnoc.DucBlockControl(self.graph.get_block(last_block_out_chain)),
                last_port_out_chain,
            )
        else:
            self.duc = None
        ...
        if self.duc:
            uhd.rfnoc.connect_through_blocks(
                self.graph,
                last_block_out_chain,
                last_port_out_chain,
                self.tx_radio.get_unique_id(),
                self.tx_chan,
            )
...
def check_settings(settings: any) -> None:
    ...
    for member in RfSettings._fields:
        if member not in settings_members:
...
def check_graph_settings(graph_settings: any) -> None:
    ...
    for member in GraphSettings._fields:
        if member not in graph_settings_members:
...
def configure_rx(session: ExampleSession, rx_settings: any) -> None:
    ...
    if session.ddc:
        print(
            f"Reading DDC block output rate: {session.ddc.block.get_output_rate(session.ddc.port)}"
        )
...
def receive_data(session: ExampleSession, buffer: np.ndarray, buffer_size: int) -> tuple:
    ...
    # receive the data
    received = session.rx_streamer.recv(buffer, rx_md, timeout)
...
if __name__ == "__main__":
    ...
    graph_settings = GraphSettings(rx_radio=0, rx_chan=0, tx_radio=0, tx_chan=0)
    rx_settings = RfSettings(frequency=2e9, gain=0, antenna="RX1", rate=1e6)
    tx_settings = RfSettings(frequency=2e9, gain=0, antenna="TX/RX0", rate=1e6)
